...
/** Enrich the annotated type with all properties of `T` set to mandatory (unwrapped with `js.UndefOr`). Methods ('def`)
  * are also added to the annotated type, but the return type is not modified.
  *
